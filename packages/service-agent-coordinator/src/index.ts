 ...
 *
 * Coordinator Agent Service 启动入口
 */
...
// 从当前文件位置加载 .env 文件
// 优先加载 packages/service-agent-coordinator/.env，如果不存在则加载根目录 .env
...
app.listen(port, () => {
  logger.server.info(`Coordinator Agent Service is running on http://localhost:${port}`);
});
